...
function stopSuck()
  if GetDI("DI_UNDER") == 0 then
    S_Error.str = "nothing under"
    N_Error.num = n_function + 12
    Stop()
  end
  
  if WaitDI("DI_AIR_LEFT",0,3000) == true then
    S_Error.str = "-- "
    N_Error.num = n_function + 13
    Stop()
  end
  if WaitDI("DI_AIR_RIGHT",0,3000) == true then
    S_Error.str = "--"
    N_Error.num = n_function + 14
    Stop()
  end
  SetDO("DO_SUCK",0)
  if WaitDI("DI_AIR_LEFT",1,3000) == true then
    S_Error.str = "--"
    N_Error.num = n_function + 15
    Stop()
  end
  if WaitDI("DI_AIR_RIGHT",1,3000) == true then
    S_Error.str = "--"
    N_Error.num = n_function + 16
    Stop()
  end
