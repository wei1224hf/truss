...
function grabToRGV()
  ...
  if (PR_Rob_pos_put.num < 1) or (PR_Rob_pos_put.num > 5) then
  ...
  local _RGV_P = _RGV_PS[PR_Rob_pos_put.num]

  Knext = CopyJointTarget(_RGV_P)
  Knext.robax.rax_3 = n_safeHeight
  MoveAbsJ(Knext,v200,fine,tool0,wobj0,load0)
  Sleep(1000)
  Knext.robax.rax_3 = _RGV_P.robax.rax_3 + 166 - N_productHeight.num
...
function grabFromIn()
  ...
  local _y = 4 - math.ceil( (PR_destackingNum.num - ((Z - 1) * 20))/5) + 1
  local _x = 5 - (PR_destackingNum.num - (Z - 1) * 20 - (4 - Y) * 5) + 1
...
function getConfig()
  ...
  for var=1,2,1 do
    PR_Tray_Num.num = var
    local readfile = io.open("/home/controller/usr/Program/pos"..PR_Tray_Num.num..".json","r")
    ...
    for var2=1,4,1 do
      p_[PR_Tray_Num.num][var2].robax.rax_1 = posistions[var2].robax.rax_1
      p_[PR_Tray_Num.num][var2].robax.rax_2 = posistions[var2].robax.rax_2
    end
  end 
  ...
  for var=3,6,1 do
    ...
    readfile:close()
    local posistions = pam_json.decode(readf)
    out_poses[PR_Tray_Num.num] = posistions;
    TPWrite("x:"..out_poses[PR_Tray_Num.num][1].robax.rax_1.."/y:"..out_poses[PR_Tray_Num.num][1].robax.rax_2)
  end
...
function _main()
  ...
  while true do
    Sleep(10)
    if GetDO("DO_PgReset") == 0 then
      Sleep(10)
    else
      SetDO("DO_PgReset",0)
      if PR_ProductCategory.num == 1 then
        N_productHeight.num = 166
      end
      if PR_Pro_Num.num == 2 then
        visionOnConveyor()
        WaitDO("DO_AllowCrawl",1,60000,false)
        grabFromConveyor()
        WaitDO("DO_AllowCrawl",0,60000,false)
...
NUMDATA("PR_Pro_Num",0)
NUMDATA("PR_Tray_Num",0)
NUMDATA("PR_Rob_pos_put",0)
NUMDATA("PR_ProductCategory",0)
NUMDATA("PR_layers",0)
NUMDATA("PR_Buffer_Num",0)
NUMDATA("PR_destackingNum",0)
