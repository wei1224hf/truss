...
function visionOnIn()
  ...
  if (N_V_step.num > 5) or  (N_V_step.num < 1) then
    S_Error.str = "vision step is wrong : "..N_V_step.num
    N_Error.num = n_visionOnIn + 4
    Stop()
  end
  ...
  if N_V_step.num == 1 then
    ...
    if N_VX.num == -99999 then
      S_Error.str = "vision unsucess"
      N_Error.num = n_visionOnIn + 6
      Stop()
    end
    local previous = CopyJointTarget(p_v[N_In.num][N_V_step.num  - 1])
     
    local _current = GetJointTarget("Xyzw")
    local distance = getDistanceByK(_current,previous)
  
    if distance > 10 then
      local xyd = "x:"..current.robax.rax_1.."/y:".._current.robax.rax_1.."/d:"..distance;
      TPWrite(xyd)
      S_Error.str = "current position is unsuitable:" .. xyd
      N_Error.num = n_visionOnIn + 5
      Stop()
    end 
    
    local _p = p_[N_In.num][N_V_step.num - 1]
    _p.robax.rax_1 = previous.robax.rax_1 - Kconveyor_V.robax.rax_1 + N_VX.num
    _p.robax.rax_2 = previous.robax.rax_2 - Kconveyor_V.robax.rax_2 + N_VY.num
    N_VX.num = -99999
  end
  
  if  N_V_step.num < 5 then  
    Knext = CopyJointTarget(p_v[N_In.num][N_V_step.num])
    MoveAbsJ(Knext,v100,fine,tool0,wobj0,load0)
    SetDO("DO_LIGHT",1)
  end
...
function visionOnOut()
  ...
  if (N_V_step.num > 5) or  (N_V_step.num < 1) then
    S_Error.str = "vision step is wrong"
  ...
  if N_V_step.num == 1 then
    ...
    local previous = CopyJointTarget(p_v[N_Out.num][N_V_step.num  - 1])
    local _current = GetJointTarget("Xyzw")
    local distance = getDistanceByK(_current,previous)
    if distance > 10 then
      S_Error.str = "current position is unsuitable"
      N_Error.num = n_visionOnOut + 5
      Stop()
    end
  end
