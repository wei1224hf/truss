...
function grabToOut()
  ...
  local _z = _z_[N_Out.num].num
  local _i = _i_[N_Out.num].num
  ...
  local p_5 = p_out[N_Out][1]
  local p_8 = p_out[N_Out][2]
  
  local _p_5 = CopyJointTarget( p_5 )
  _p_5.robax.rax_1 = _p_5.robax.rax_1 + offset_out[N_Out][1][1].num
  _p_5.robax.rax_2 = _p_5.robax.rax_2 + offset_out[N_Out][1][2].num
  
  local _p_8 = CopyJointTarget( p_8 )
  _p_8.robax.rax_1 = _p_8.robax.rax_1 + offset_out[N_Out][2][1].num
  _p_8.robax.rax_2 = _p_8.robax.rax_2 + offset_out[N_Out][2][2].num
  ...
  if _i == 8 then
    Knext = CopyJointTarget(_p_8)
  elseif _i == 5 then
    Knext = CopyJointTarget(_p_5)
  elseif _i == 4 then
...
function suckFromShelf()
  ...
  if GetDI("DI_UNDER") == 1 then
    S_Error.val = "there is a panel stuck on the shelf, remove it"
    N_Error.num = N_suckFromShelf.num + 2
    Stop()
    return 0
  end
  
  --TODO DI check for grab
...
NUMDATA("N_grabToCache",100)
NUMDATA("N_grabFromCache",100)
NUMDATA("N_grabToCart",100)
NUMDATA("N_toWait",100)
NUMDATA("N_grabFromConveyor",100)
NUMDATA("N_upFromConveyor",100)
NUMDATA("N_grabToOut",100)
NUMDATA("N_grabToRGV",100)
NUMDATA("N_suckFromShelf",100)
NUMDATA("N_suckToOut",100)
NUMDATA("N_visionOnIn",100)
NUMDATA("N_visionOnOut",100)
NUMDATA("N_visionOnRGV",100)
